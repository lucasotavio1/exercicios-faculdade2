...
class Pedido:
    ...
    def adicionarProduto(self, produto: Produto, quantidade: int):
        subtotal = produto.preco * quantidade
        item = ItemPedido(produto.codigo, produto.nome,
                          produto.descricao, produto.preco, quantidade, subtotal)
        return item

    def calcularTotal(self, itens_pedido: list[ItemPedido]):
        total_calculado = 0.0
...
class Carrinho:
    ...
    def removerItem(self, item: ItemPedido):
        self.itens.remove(item)

    def checkout(self):
        pedido = Pedido(id=1, data=date.today())
        total_pedido = pedido.calcularTotal(self.itens)
...
class Pagamento:
    ...
    def realizarPagamento(self, forma_pagamento) -> bool:
...
class PagamentoOnline(Pagamento):
    ...
    def realizarPagamento(self) -> bool:
        print(
